-- =============================================================================
-- Social Post Star Database Schema
-- =============================================================================
-- This file contains the complete database schema for the Social Post Star app.
-- Run this to create all necessary tables, indexes, and constraints.
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- content_items: Stores all content from Webflow, HubSpot, and other sources
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- generated_posts: All posts created by the system
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS generated_posts (
  ...
  campaign_id UUID, -- For grouping campaign posts
...
-- -----------------------------------------------------------------------------
-- post_history: Tracks when content was used in posts (for duplication prevention)
-- -----------------------------------------------------------------------------
...
-- Composite index for duplication checking (most common query)
CREATE INDEX idx_post_history_duplication_check ON post_history(content_id, used_date DESC);

-- -----------------------------------------------------------------------------
-- user_feedback: Stores user feedback on generated posts (for learning)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- industry_articles: RSS-discovered industry articles
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- special_days: Holidays and industry celebration days
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- content_taxonomy: AI-built understanding of content topics/themes
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- brand_voice_embeddings: Vector embeddings for RAG system
-- -----------------------------------------------------------------------------
-- Note: For now using JSONB to store embeddings. If using pgvector extension:
-- ALTER TABLE brand_voice_embeddings ALTER COLUMN embedding TYPE vector(1536);
CREATE TABLE IF NOT EXISTS brand_voice_embeddings (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  source brand_voice_source NOT NULL,
  text TEXT NOT NULL,
  embedding JSONB NOT NULL, -- Array of numbers, later can be vector type
  metadata JSONB DEFAULT '{}',
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);
...
-- View: Posts with content information
...
COMMENT ON TABLE content_items IS 'Stores all content from various sources (Webflow, HubSpot, etc.)';
COMMENT ON TABLE generated_posts IS 'All social media posts generated by the AI system';
COMMENT ON TABLE post_history IS 'Tracks content usage for duplication prevention';
COMMENT ON TABLE user_feedback IS 'User accept/reject feedback for AI learning';
COMMENT ON TABLE industry_articles IS 'RSS-discovered articles relevant to industry';
COMMENT ON TABLE special_days IS 'Holidays and industry celebration days';
COMMENT ON TABLE content_taxonomy IS 'AI-generated understanding of content topics';
COMMENT ON TABLE brand_voice_embeddings IS 'Vector embeddings for brand voice RAG system';
